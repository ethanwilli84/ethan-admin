...
export default function LeadPoolPage() {
  ...
  const [activeTab, setActiveTab] = useState<'pool' | 'ingest' | 'csv'>('pool')
  ...
  useEffect(() => {
    loadStats()
    loadLeads()
    fetch('/api/campaigns').then(r => r.json()).then(d => setCampaigns(d.map((c: { slug: string; name: string }) => ({ slug: c.slug, name: c.name }))))
  }, [])
  ...
  async function loadLeads() {
    ...
    const r = await fetch(`/api/lead-pool?${params}`)
    const d = await r.json()
    if (d.ok) setLeads(d.leads)
    setLoading(false)
  }

  async function ingest(sourceType: string, extra: Record<string, unknown> = {}) {
    setIngesting(true)
    ...
    const res = await fetch('/api/lead-sources', {
      ...
      body: JSON.stringify({ sourceType, campaigns: selectedCampaigns, ...extra })
    })
    const d = await res.json()
    setIngestLog(prev => [...prev, d.ok ? `✓ Added ${d.count} new leads (pool: ${d.poolSize} total)` : `✗ Error: ${d.error}`])
    setIngesting(false)
    loadStats(); loadLeads()
  }
    ...
    <div>
      ...
      <div className="main">
        ...
        {stats && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 10, marginBottom: 20 }}>
            {[
              ...
              { label: 'Sources', value: stats.sources.length, color: 'var(--text-2)' },
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          {(['pool', 'ingest', 'csv'] as const).map(t => (
        ...
        {activeTab === 'ingest' && (
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 16, lineHeight: 1.7 }}>
              Pull leads from structured databases into the pool. These replace Claude web research — deterministic, no duplicates, scales infinitely.
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[
                { id: 'itunes_podcasts', icon: '🎙', title: 'iTunes Podcasts', subtitle: 'Free · unlimited · 100s of podcasts per query with RSS emails', color: '#ff0000' },
                { id: 'fdic_banks', icon: '🏦', title: 'FDIC Bank Database', subtitle: 'Free · 4,000+ community banks · perfect for lending campaigns', color: '#00C896' },
              ].map(src => (
                <div key={src.id} style={{ background: 'var(--surface-2)', borderRadius: 10, padding: 16, border: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 24, marginBottom: 8 }}>{src.icon}</div>
                  <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>{src.title}</div>
                  <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 12 }}>{src.subtitle}</div>
                  <button className="btn-primary" style={{ fontSize: 12 }} disabled={ingesting} onClick={() => ingest(src.id)}>
                    {ingesting ? '◌ Fetching...' : 'Fetch Leads →'}
                  </button>
                </div>
              ))}
            </div>
            {ingestLog.length > 0 && (
              <div style={{ marginTop: 14, background: '#0d0d0d', borderRadius: 8, padding: 12, fontFamily: 'var(--font-dm-mono)', fontSize: 12 }}>
                {ingestLog.map((l, i) => <div key={i} style={{ color: l.includes('✓') ? '#4ade80' : l.includes('✗') ? '#f87171' : '#d1d5db' }}>{l}</div>)}
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'csv' && (
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 12 }}>
              Upload any CSV with leads — Apollo.io exports, manual lists, scraped data. Needs: name, website columns. Optional: email, category, description.
            </div>
            ...
            {csvContent && <div style={{ fontSize: 11, color: 'var(--text-3)', fontFamily: 'var(--font-dm-mono)', marginBottom: 12 }}>
            ...
            <input value={csvSource} onChange={e => setCsvSource(e.target.value)} placeholder="Source label (e.g. apollo_export, manual_list)"
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)' }}>
                    {['Name', 'Category', 'Email', 'Source', 'Campaigns', 'Status', 'Score'].map(h => (
                ...
                <tbody>
                  {leads.map(lead => (
                    <tr key={lead._id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '7px 10px', color: 'var(--text-3)', fontSize: 10 }}>{(lead.campaigns || []).length}</td>
